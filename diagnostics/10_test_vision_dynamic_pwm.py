"""实际vision.process_frame与四路动态PWM组合测试，不执行PID或窗口显示。"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="vision.py + camera + dynamic PWM test")
...
def main():
    ...
    try:
        ...
        print(
            f"视觉组合测试开始：camera={args.device}, 640x480, pwm={args.frequency}Hz, "
            f"update_hz={args.update_hz}, duration={args.duration}s"
        )
        ...
        while time.monotonic() - started_at < args.duration:
            ...
            if ok:
                frames += 1
                consecutive_failed = 0
                position, cx, _cy, _mask, _annotated, _area = process_frame(frame)
            ...
            if now >= next_report:
                ...
                print(
                    ...
                    f"frames={frames} failed={failed} detections={detections} "
                    f"updates={updates} position={last_position}"
                )
                next_report = now + 1.0
    ...
    print(
        f"视觉组合测试结束：elapsed={elapsed:.1f}s frames={frames} failed={failed} "
        f"detections={detections} updates={updates}，资源已清理"
    )
